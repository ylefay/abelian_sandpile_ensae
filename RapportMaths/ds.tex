...
Posons $E=\{-1,0,1\}$ et $d=1$, le graphe correspondant est composé de la maille suivante \begin{center} \begin{tikzpicture}[main/.style = {draw, circle}] 
...
\begin{lemma}
	...
	\begin{proof}
		...
		\end{align*} 
	\end{proof}\end{lemma}
	Dans le cas particulier où la quantité $\varphi^*=C$ est constante (par exemple $C= 1/c$), le lemme précédent dit que $p(s) = C^t\textup{Card}\{(\delta_i)\in \mathcal{E}^t : \sum_{i=0}^{t-1}\delta_i=x\}$. %En se restreignant à $d=1$ et au cas où $\varphi^*$ est constante, nous avons le lemme suivant,
	%\begin{lemma}
